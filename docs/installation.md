...
4. **Danh tính và lưu trữ**: nhãn node, cổng gateway, thư mục dữ liệu (local) hoặc tên miền
   (docker-public).
...
Mở `http://127.0.0.1:8765`. Bearer token nằm trong `./.data/identity.json` (trường
`localToken`). Node mặc định chỉ bind loopback và từ chối bind địa chỉ công khai nếu thiếu
`--allow-public-bind`.
