...
- **Nội dung tệp tới agent**: tệp văn bản đi vào prompt của lượt (`attachmentBrief` chèn nội dung, `read_attachment` đọc lại theo id), và một journey chứng minh câu trả lời **dùng** nội dung đó ([attachments](../apps/web/e2e/attachments.spec.ts), "the agent answers using the content of an attached file"). **Chưa có extractor cho PDF và ảnh**: chúng được nêu bằng id chứ không được đọc, và `prompt(sessionId, text)` chỉ nhận văn bản, nên nội dung ảnh/PDF chưa tới model. Điều kiện còn thiếu: một extractor, và một adapter nhận được nội dung ảnh trong prompt.
