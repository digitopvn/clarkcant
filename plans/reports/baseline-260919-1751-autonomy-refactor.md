...
## 16. Ship `main --merge`: hoà giải với main — trạng thái bàn giao (chưa xong)

Yêu cầu: ship nhánh này lên `main` kèm merge. Pre-flight cho thấy **nhánh sau main 131 commit** (remote `digitopvn/clarkcant`, default `main`), 21 commit / 63 file / +8574−273. Giữ nguyên theo lựa chọn của người dùng: hoà giải trên nhánh nháp, ship trọn gói một PR. Nhánh backup đã tạo: **`backup/pre-reconcile-260920` ở `d54a832`** (bản refactor đã verify xanh: 1281 unit pass, e2e 56/56).

**Vì sao không merge máy móc.** `git merge --no-commit --no-ff origin/main` xung đột đúng 15 file như `merge-tree` dự đoán, và hai va chạm là cấu trúc chứ không phải văn bản: (1) main dùng `version: 18` cho migration `memory-records` còn nhánh này dùng 18 cho `secrets`; (2) main **xoá** `packages/conversation-client/src/SettingsPanel.tsx` và chuyển settings vào thư mục `settings/` (thêm tab Memory), nên phần Autonomy tab / bảng model pool / fallback provider của nhánh này phải **port**, không phải giải xung đột. Ngoài ra main đã có sẵn block `question-card` riêng trong `node-tools.ts` (nhưng không có `ask_user_question`) và một mô hình policy riêng (`ExecutionMode`/`ExecutionRule` + `readExecutionPolicy` + `decideExecution`).

**Quyết định của người dùng (đã hỏi, đã chốt).** `ExecutionPolicy` của nhánh này (`auto|guarded|confirm|deny` qua Settings → Autonomy, preflight + guardrail) là lớp quyết định cho `run_command`; effect ledger của main (`recordEffectExecution`) được **giữ** vì nó trả lời câu khác ("hiệu ứng đã đi được tới đâu"); mô hình `ExecutionMode`/`decideExecution` của main trở thành không dùng và phải được nói rõ là bị thay thế, không phải bỏ quên. Người dùng cũng chọn làm tiếp tới khi verify xanh.

**Đã làm trong cây merge (chưa commit, chưa push).** Giải hết marker xung đột ở cả 15 file: `migrate.ts` (giữ 18 memory-records, secrets=19, audit_log=20 — đã kiểm 20 version liền mạch, không trùng), `AGENTS.md` (giữ cả đoạn "definition of done" của main và yêu cầu giải phóng port), `contracts/index.ts`, ba spec e2e, hai spec runtime, `gateway.ts` (giữ cả route autonomy/stop của nhánh này lẫn route voice capabilities của main, đổi guard `/capabilities` sang dạng `segments` của main), `node-tools.ts` (giữ xương sống policy của nhánh này, thêm `effectAudit` giữ effect ledger của main, lấy mô tả và promptSnippet của main, gộp import), `main.ts` (viết lại `extraTools` thành một hàm hợp nhất), `voice-session.ts`, `blocks.tsx` (lấy re-export `HOST_OWNED_BLOCK_TYPES` từ contracts), `onboarding.spec.ts` (bản viết lại của main thắng ở khúc wizard đã bị main bỏ), `appearance.spec.ts` (đếm tab theo `SETTINGS_TABS.length`), `j1.spec.ts`.

**Còn lại, theo đúng thứ tự.** (1) `apps/runtime/src/voice-session.ts` còn 75 lỗi tsc: các edit xoá marker của tôi ghép sai hai phía trong nhánh câu hỏi (khoảng dòng 565–600) và trong hai khối state machine (khoảng 626–650) — cần đọc hai phía rồi viết lại thành hai nhánh tách biệt. (2) Một `}` thiếu ở `gateway.ts` (~2674) và một ở `main.ts` (~1653), hệ quả của cùng kiểu edit. (3) `voice-gateway.spec.ts` 54 lỗi là cascade của (1). (4) Bỏ `readExecutionPolicy` khỏi import `main.ts` (đã thành không dùng sau quyết định policy). (5) **Port settings** vào `conversation-client/src/settings/`: tab Autonomy, bảng model pool, fallback provider — việc thật, cần test. (6) Tính lại `docs/manifest.json`. (7) `pnpm verify:full` trên cây merge. (8) Commit merge, push, PR 7 mục evidence, rồi `ak:review-pr --merge`.

**Ghi chú vận hành.** Hook bảo mật chặn ghi file bằng script (fail closed) nên mọi thay đổi file phải qua công cụ edit — script chỉ được đọc. `.github/workflows/ci.yml` (39 cảnh báo line-length của lens) **không thuộc PR này**: `eslint` bỏ qua file ("no matching configuration"), repo không có yamllint/markdownlint, và `git diff backup/pre-reconcile-260920..HEAD` không đụng tới nó — nó đến từ main.

**Trạng thái ship: DỪNG, chưa publish.** Không commit, không push, không PR. Bản refactor đã verify nằm nguyên ở `backup/pre-reconcile-260920` (`d54a832`) và ở `HEAD` trước merge, nên không có gì bị mất; muốn bắt đầu lại thì `git reset --hard backup/pre-reconcile-260920 && git merge origin/main`.
