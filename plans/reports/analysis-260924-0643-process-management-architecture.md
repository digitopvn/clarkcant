...
- **P2: xong, trừ hai điểm có lý do.**
  - Đã làm: thu hồi Main Pi session rảnh, `runningMs` thật, trạng thái `stopped`, bộ quét lease định kỳ, budget `maxWallClockMs` và `maxTokens` của task, outbox backoff (5 giây nhân đôi, trần 15 phút) và dead-letter sau 12 lần.
  - Cancel sang peer mới xong **phía gửi**: `cancelTask` đưa `cancel.request` vào outbox. Production chưa có vòng gửi outbox và node nhận chưa tạo task từ `delegate`, nên phía nhận chưa có worker nào để dừng; nối phần này thuộc về phase làm việc thực thi xuyên node.
