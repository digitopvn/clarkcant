...
## Closing the audit rework

**The CI objection, and its real cause.** The auditor found the main-branch runs for merge commits `e0871aeb` (#139) and `cef02344` (#149) `cancelled` rather than green. The cause is the workflow's `concurrency` group with `cancel-in-progress`: every new commit to `main` cancels the run still in flight for the previous one. Because each phase merge was immediately followed by a docs commit to `main`, the merge commit's own run was always the one cancelled — no run ever failed. A secondary self-inflicted cause is recorded honestly: re-running an OLD run via the API re-enters the same concurrency group and therefore cancels the NEWEST run, which is why the head briefly showed `cancelled` too. The remedy is to stop re-running superseded runs, make every remaining main commit, and let a single run on the current head finish; that run covers every merge commit in its ancestry.
