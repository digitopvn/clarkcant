...
import { FileVaultBackend, MemoryVaultBackend, driverUsable, probePlatformCapabilities, redactForLog } from "@clarkcant/host-adapters";
...
describe("credential vault (T34)", () => {
  ...
  it("redacts token-shaped and JWT-shaped values from log lines", () => {
    const line = "auth failed for token sk-abcdefghijklmnop and eyJhbGciOi.eyJzdWIiOi.SflKxwRJ and access_token=abcdef123456";
    const redacted = redactForLog(line);
    expect(redacted).not.toContain("sk-abcdefghijklmnop");
    expect(redacted).not.toContain("eyJhbGciOi.eyJzdWIiOi.SflKxwRJ");
