...
describe("what a record says about itself", () => {
  it("each kind is named in words a person reads", () => {
    expect(MEMORY_KIND_LABELS.preference).toBe("Sở thích");
    expect(MEMORY_KIND_LABELS["project-fact"]).toBe("Dự án");
    expect(MEMORY_KIND_LABELS.decision).toBe("Quyết định");
    expect(memoryRow(record({ kind: "preference" }), NOW).kindLabel).toBe("Sở thích");
  });

  it("how long ago is computed rather than written down", () => {
    expect(timeLabel(NOW, NOW)).toBe("hôm nay");
    expect(timeLabel("2026-09-18T12:00:00.000Z", NOW)).toBe("hôm qua");
    expect(timeLabel("2026-09-16T12:00:00.000Z", NOW)).toBe("3 ngày trước");
    // Past a week the date is more use than a count of days.
    expect(timeLabel("2026-08-01T12:00:00.000Z", NOW)).toBe("2026-08-01");
  });

  it("the scope says whether it is about the person or about this conversation", () => {
    expect(scopeLabel("node")).toBe("Mọi cuộc trò chuyện");
    expect(scopeLabel("conversation")).toBe("Chỉ cuộc trò chuyện này");
  });

  it("the source names where it came from without quoting it", () => {
    const fromMessage = sourceLabel(record({ sourceMessageId: "msg_abcdefghijklm" }));
    ...
    const fromConversation = sourceLabel(record());
...
describe("grouping a list", () => {
  ...
  it("an empty list is an empty view rather than a list of empty sections", () => {
    expect(memoryView([], NOW)).toEqual({ groups: [], count: 0 });
  });
});
