...
export interface TerminalRegistry {
  ...
  /** Put text on the prompt without pressing Enter. Refused while something is running. */
...
if [ "\${BASH_VERSINFO[0]}" -gt 4 ] || { [ "\${BASH_VERSINFO[0]}" -eq 4 ] && [ "\${BASH_VERSINFO[1]}" -ge 4 ]; }; then
  PS0='\\033]133;C;$(__cc_command)\\007'"\${PS0:-}"
  ...
  __cc_preexec() {
    ...
    __cc_armed=
    printf '\\033]133;C;%s\\007' "$(__cc_command)"
  }
...
__cc_preexec() { local c="\${1//[[:cntrl:]]/ }"; printf '\\033]133;C;%s\\007' "$c"; }
__cc_precmd() { local s=$?; printf '\\033]133;D;%s\\007\\033]133;A\\007' "$s"; return $s; }
...
export function createTerminalRegistry(options: {
  ...
  const collect = (terminal: Terminal, text: string): void => {
    ...
    if (current === null) return;
    current.raw += text;
    if (current.raw.length > TERMINAL_LIMITS.commandRawChars) {
  ...
  return {
    ...
    async open(input) {
      prune();
      const running = [...terminals.values()].filter((terminal) => terminal.info.status === "running").length;
    ...
    kill(terminalId) {
      ...
      if (terminal === undefined || terminal.info.status !== "running") return false;
      try {
        terminal.pty.kill(platform === "win32" ? undefined : "SIGHUP");
      } catch {
        // Already gone; the exit handler has or will record it.
      }
      // A shell that ignores the hangup is killed outright: closing a terminal must close it.
      terminal.killTimer = setTimeout(() => {
        if (terminal.info.status !== "running") return;
        try {
          terminal.pty.kill(platform === "win32" ? undefined : "SIGKILL");
        } catch {
